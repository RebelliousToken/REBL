        ...
        model(0)
{
    ...
    ui->REBLAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaREBLLabel->setTextFormat(Qt::RichText);
...
//    connect(ui->TradingTabWidget, SIGNAL(tabBarClicked(int)), this, SLOT(on_TradingTabWidget_tabBarClicked(int)));
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "REBL(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "REBL(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(REBL)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //REBLst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
    ...
    QString Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarket/REBL_BTC");
    return Response;
}

QString tradingDialog::GetOrderBook(){

    QString  Response = sendRequest("https://www.cryptopia.co.nz/api/GetMarketOrders/REBL_BTC");
    return Response;
}
...
void tradingDialog::GetMarketHistory(){
    ...
    sendRequest1(QString("https://www.cryptopia.co.nz/api/GetMarketHistory/REBL_BTC"), f);
}
...
QString tradingDialog::BuyREBL(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://www.cryptopia.co.nz/api/SubmitTrade";

    QJsonObject stats_obj;
    stats_obj["Market"] = "REBL/BTC";
    stats_obj["Type"] = "Buy";
...
QString tradingDialog::SellREBL(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString Response = sendRequest(URL, "POST", QString("{\"Currency\":\"REBL\"}"));
    return Response;
}
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>REBL Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> REBL");
...
void tradingDialog::AskInfoInsertSlot(int row, int col){
    ...
    QString UnitsREBLString = ui->AsksTable->model()->data(ui->AsksTable->model()->index(row,1)).toString();
    ui->SellBidPriceEdit->setText(SellBidPriceString);
    ui->UnitsInputREBL->setText(UnitsREBLString);
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->REBLSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(REBLSupply,'i',8) + "</span><b> REBL</b>");
    ...
    ui->REBLDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(REBLDemand,'i',8) + "</span><b> REBL</b>");
...
void tradingDialog::ParseAndPopulatePriceChart(QString Response){
    ...
    derPlot->yAxis->setLabel("REBL Price in BTC");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
        case 0:    //buy tab is active
            ...
            f = std::bind(&tradingDialog::showBalanceOfREBLOnTradingTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "REBL" + QString("\"}"));
            ...
            sendRequest1("https://www.cryptopia.co.nz/api/GetMarketOrders/REBL_BTC", f);
        ...
        case 1: //Cross send tab active
            ...
            f = std::bind(&tradingDialog::showBalanceOfREBLOnSendTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "REBL" + QString("\"}"));
        ...
        case 5://show balance tab
            ...
            f = std::bind(&tradingDialog::showBalanceOfREBLOnBalanceTab, this);
            sendRequest1(QString("https://www.cryptopia.co.nz/api/GetBalance"), f, "POST", QString("{\"Currency\":\"") + "REBL" + QString("\"}"));
...
void tradingDialog::showBalanceOfREBLOnTradingTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->REBLAvailableLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfREBLOnSendTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->CryptopiaREBLLabel, Response);
    }
}
...
void tradingDialog::showBalanceOfREBLOnBalanceTab() {
    QNetworkReply *reply = qobject_cast<QNetworkReply *>(sender());
    QString Response = "";
    if (reply->error() == QNetworkReply::NoError) {
        //success
    ...
    reply->deleteLater();
    if(Response.size() > 0 && Response != "Error"){
        DisplayBalance(*ui->REBLBalanceLabel,*ui->REBLAvailableLabel_2,*ui->REBLPendingLabel, QString::fromUtf8("REBL"),Response);
    }
}
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputREBL->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("REBL");
    ...
    double AvailableREBL = BalanceObject["Available"].toDouble();
...
{
    //calculate amount of BTC that can be gained from selling REBL available balance
    QString responseA = GetBalance("REBL");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONArray(responseA);

    double AvailableREBL = ResultObject["Available"].toDouble();

    ui->UnitsInputREBL->setText(str.number(AvailableREBL,'i',8));
}
...
{
    double Quantity = ui->CryptopiaREBLLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableREBL,'i',8));
}
...
void tradingDialog::on_BuyREBL_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyREBL(Order,Quantity,Rate);
...
void tradingDialog::on_SellREBLBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputREBL->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellREBL(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_CSUnitsBtn_clicked()
{
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        if ( ((Quantity / x) - y) > 0 )
        {
            ...
            QString SellResponse = SellREBL(Order,y,x);
            ...
            QString SellResponse = SellREBL(Order,(Quantity / x),x);
            QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
            QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
            ...
            if (SellResponseObject["Success"].toBool() == false){
                ...
                if (ResponseObject["Success"].toBool() == false){
                    ...
                    if (ResponseObject["Success"].toBool() == false){
                        ...
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" REBL for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
                } else if (ResponseObject["Success"].toBool() == true){
                    QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" REBL for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                }
...
void tradingDialog::on_UnitsInputREBL_textChanged(const QString &arg1)
{
    CalculateSellCostLabel(); //update cost
}
