...
// Implemented REBL parallel miner | Auto deploy
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("REBLMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("REBLMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("REBLMiner started (%s)\n", (fProofOfStake ? "PoS" : "PoW"));
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rebellious-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running REBLMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
