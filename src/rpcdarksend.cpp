...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Rebelliousaddress> <amount>\n"
            "Rebelliousaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <Rebelliousaddress> <amount>\n"
            "Rebelliousaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Rebellious address");
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "start-index")
    {
        ...
        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            if(mne.getIndex() == index) {
                c = pwalletMain->mapMyRebelliousNodes[mne.getIndex()];
                found = true;
                std::string errorMessage;
