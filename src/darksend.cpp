    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Rebellious and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Rebellious per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeRebelliousAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fRebelliousSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f Rebellious CENT - sessionTotalValue %d\n", fRebelliousSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100Rebellious+1 ( bit on if present )
    // bit 1 - 10Rebellious+1
    // bit 2 - 1Rebellious+1
    // bit 3 - .1Rebellious+1
    // bit 3 - non-denom
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("Rebellious-darksend");
    ...
    while (true)
    {
        ...
        LogPrintf("Masternode list count %i \n",RequestedMasterNodeList);
        //try to sync the masternode list and payment list every 5 seconds from at least 3 nodes
        if(c % 3 == 0 && RequestedMasterNodeList < 3){
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeRebelliousAmount = (rand() % nLeftToAnon)+3;
                } else {
